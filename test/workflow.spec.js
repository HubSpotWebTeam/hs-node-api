...
const { schemaWorkflows, schemaWorkflow, validate } = require('./schemas/workflows');

const { E2E_TESTS_HAPI_KEY: hapikey, E2E_TESTS_WORKFLOW_ID, E2E_TESTS_CONTACT_EMAIL: email, E2E_TESTS_CONTACT_ID: vid } = process.env;
...
describe.only('Workflows', async () => {
  describe('Retrieving Workflows', async () => {
    ...
    it('returns a valid workflow', async () => {
      const workflowById = await hs.workflows.getWorkflow(E2E_TESTS_WORKFLOW_ID);
  ...
  describe('Mutating Workflows', async () => {
    it('creates a new workflow', async () => {
      ...
      expect(newWorkflow.name).to.be.a('string').and.to.equal(name);
      expect(newWorkflow.type).to.be.a('string').and.to.equal(type);
      expect(newWorkflow.actions).to.be.an('array').and.to.have.lengthOf(3);
      return Promise.resolve();
    });
    ...
    it('updates a workflow with new actions', async () => {
      ...
      const updatedWorkflowProperties = Object.assign({}, createdWorkflow, {
        actions: createdWorkflow.actions.concat({ type: 'DELAY', delayMillis: 3600000 })
      });

      const updatedWorkflow = await hs.workflows.updateWorkflow(updatedWorkflowProperties);
      expect(updatedWorkflow.actions).to.be.an('array').and.to.have.lengthOf(4);
      return Promise.resolve();
    });
  });
  ...
  describe('Enrolling/unenrolling contacts in workflows', async () => {
    it('enrolls a contact in to an existing workflow', async () => {
      const enrollment = await hs.workflows.enrollContact({ workflowId: createdWorkflow.id, email });
    ...
    it('gets a list of enrollments for a specified contact and should have at least one (1) enrollment', async () => {
      ...
      expect(enrollments).to.be.an('array', 'Enrollments is not an array').and.to.have.lengthOf.at.least(1, 'Must have at least 1 enrollment');
      return Promise.resolve();
    });

    it('unenrolls a contact from a pre-existing workflow', async () => {
      const enrollment = await hs.workflows.unenrollContact({ workflowId: createdWorkflow.id, email });
      expect(enrollment).to.be.a('object');
  ...
  describe('Retrieving event logs', async () => {
    ...
    xit('retrieves an event log for a specific workflowId', async () => {
      const eventLogs = await hs.workflows.getWorkflowEventLog({ workflowId: createdWorkflow.id });
      debug(eventLogs);
  ...
  describe('Deleting Workflows', async () => {
    it('deletes a workflow', async () => {
      const deletedWorkflow = await hs.workflows.deleteWorkflow(createdWorkflow.id);
